...
namespace Dargon.Nest {
   public class LocalDargonBundle : IDargonBundle {
      private const string kRemoteSettingName = "REMOTE";
      private const string kChannelSettingName = "CHANNEL";
      private readonly string bundlePath;

      public LocalDargonBundle(string bundlePath) {
         this.bundlePath = bundlePath;
      }

      public string CachePath => Path.Combine(bundlePath, ".cache");
      ...
      public void UpdateEggs() => UpdateEggs(new UpdateNestOptions());

      public void UpdateEggs(UpdateNestOptions updateNestOptions) {
         ...
         for (var eggIndex = 0; eggIndex < eggsAndVersions.Length; eggIndex++) {
            ...
            var remoteEgg = RemoteEgg.FromUrl($"{remote}/{name}-{version}", $"{remote}/{name}");

            updateState.SetState($"Updating Dargon Egg `{name}`!", (double)eggIndex / eggsAndVersions.Length);
            if (!IsExistsEgg(name)) {
               InstallEgg(remoteEgg, updateState);
            } else {
               UpdateEggHelper(new LocalEgg(GetEggPath(name)), remoteEgg, updateState);
            }
         }
         updateState.SetState($"Update Complete!", 1);
      }

      public IEnumerable<Egg> EnumerateEggs() {
         foreach (var directory in Directory.EnumerateDirectories(bundlePath)) {
            LocalEgg egg = null;
            try {
               egg = new LocalEgg(directory);
            } catch (Exception e) {
               Console.WriteLine(e);
            }
            if (egg != null) {
      ...
      public void InstallEgg(Egg egg) => InstallEgg(egg, new UpdateState());

      public void InstallEgg(Egg egg, UpdateState updateState) {
         using (var nestLock = LocalNestLock.TakeLock(bundlePath)) {
            var localEggPath = Path.Combine(bundlePath, egg.Name);
      ...
      private string GetEggPath(string eggName) => Path.Combine(bundlePath, eggName);

      public void UpdateEgg(string eggName) {
         using (var nestLock = LocalNestLock.TakeLock(bundlePath)) {
            var localEggPath = Path.Combine(bundlePath, eggName);
            if (!Directory.Exists(localEggPath)) {
               throw new InvalidOperationException("Could not find egg `" + eggName + "`.");
            }

            var egg = new LocalEgg(localEggPath);
      ...
      private void UpdateEggHelper(LocalEgg localEgg, Egg remoteEgg, UpdateState updateState) {
...
//         foreach (var internalPath in allFilePaths) {
      ...
      private string GetSetting(string settingName) {
         return File.ReadAllText(Path.Combine(bundlePath, settingName), Encoding.UTF8);
      }

      private void SetSetting(string settingName, string value) {
         File.WriteAllText(Path.Combine(bundlePath, settingName), value, Encoding.UTF8);
      }
   }
}